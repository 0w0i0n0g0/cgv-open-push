...
class CgvOpenPushTest(unittest.TestCase):
    ...
    def test_send_curl_to_cgv_multiple(self):
        for data in enumerate(json_data):
            ...
            print(f"위치 : {extract_text_between_tag(response, "THEATER_NM")}")
            print(f"유형 : {extract_text_between_tag(response, "RATING_NM")}")
            print(f"영화 : {extract_text_between_tag(response, "MOVIE_GROUP_NM")}")
